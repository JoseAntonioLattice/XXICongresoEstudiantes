...
Compact formulation: $$A_{x,\mu} \in \mathfrak{su}(N) \to U_{x,\mu} \in \text{SU}(N).$$
...
Instead of working with fermions one works with \textit{pseudo-fermions}. \\~

Pseudo-fermions are bosonic variables with a similar partition function to fermion determinant for two degenerate fermion masses.
...
$$\mathcal{L} = \frac{1}{4}F_{\mu\nu}F_{\mu\nu} + \sum_f\bar\psi^f(i\gamma_{\mu}D_{\mu}-m_0)\psi^f,$$
...
\begin{frame}{Pion correlation function}
   Iso-triplet pion propagator $\left\langle O_n\bar O_m\right\rangle = C_{\text{pion}}(|n-m|)$, where $O_n = \bar d_n \gamma_5 u_n$. 
   $$C_{\text{pion}}(|n-m|) = \left\langle-\frac{1}{\sqrt{V_{\text{spatial}}}}\sum_{x,\alpha,\beta}\left | D^{-1}_{\text{Dirac}}(n|m)_{\alpha\beta}\right|^2\right\rangle.$$  
\end{frame}
...
To take into account periodicity we use
    $$\frac{C(t)}{C(t+1)} = \frac{\cosh(m_{\text{eff}}(t-N_T/2))}{\cosh(m_{\text{eff}}(t+1-N_T/2))},$$
    and solve for $m_{\text{eff}}$ at each $t$.
\end{frame}


\begin{frame}{Effective mass}
\centering
\includegraphics[scale=0.9]{meff_plot.pdf}
...
\begin{frame}{Topological susceptibility with slab method}
    ...
    from which we infer
    $$\langle q^2\rangle = \chi_tVx(1-x) + x^2Q^2. $$
\end{frame}

\begin{frame}{Topological susceptibility with slab method}
    \centering
    \includegraphics[scale=0.7]{top_char_slab_3.pdf}
